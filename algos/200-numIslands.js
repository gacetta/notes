...
const numIslands = function(grid) {
  ...
  const processIsland = (row, col) => {
    ...
    if (row < rows - 1 && grid[row + 1][col] === '1') processIsland(row + 1, col);
    if (col > 0 && grid[row][col - 1] === '1') processIsland(row, col - 1);
    if (col < cols - 1 && grid[row][col + 1] === '1') processIsland(row, col + 1);
  };
...
// // well commented leetcode solution
