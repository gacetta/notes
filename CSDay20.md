...
MongoDB is a document-oriented DB. _Not relational_
...
2. Instead of tables, you have collections of documents.
   ...
   c. You have the ability to nest arbitrarily deep. _PROCEED WITH CAUTION_
...
in mongoose, a virtual is a property that is not stored in MongoDB, instead they are typically used for computed properties.

supposed you have a `User` model and a `Task` model. You want to see all `tasks` owned by a `user`.
Rather than store a `tasks` property on the `User` model, you can create a virtual:
...
_WARNING_: by default, the `doc` obtained in the `callback` will be the found object, NOT the updated object. To fix this, specify `new: true` in options object.
